...
char* MD5Buffer(void *buff, int len)
{
	md5 alg;

	alg.Update((unsigned char*)buff, (unsigned int)len);
	alg.Finalize();
...
char* MD5File(char* szFilename)
{
	...
	try
	{
		...
		{
			while (nLen = (int)fread(chBuffer, 1, 1024, file))
			{
...
// MD5File: Performs the MD5 algorithm on a file (binar or text),
// returning the results as a char*.  Returns NULL if it fails.
char* MD5File(char* szFilename, int block)
{
	FILE* file;
	md5 alg;
	int nLen;
	...
	try
	{
		...
		if ((file = fopen (szFilename, "rb")) != NULL)
		{
			while ((nLen = (int)fread(chBuffer, 1, 1024, file)) && (index < block || block == -1))
			{
				alg.Update(chBuffer, nLen);
				index++;
			}
